...
function Hero() {
  ...
  const drawPixels = useCallback(() => {
    ...
    const size = Math.max(20, Math.min(40, Math.floor(baseSize)));
    ...
    drawnPixels.forEach((pos) => {
      ...
      if (touchingDark) {
        ctx.fillStyle = "#000000";
        ctx.fillText(random() > 0.5 ? "1" : "0", x * size + size / 2, y * size + size / 2);
      }
    });
  }, []);
...
// ✅ Export as default so it can be imported anywhere
export default Hero;