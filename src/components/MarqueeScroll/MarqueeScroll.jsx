...
const MarqueeScroll = () => {
  ...
  useEffect(() => {
    ...
    // ✅ KEEP LENIS (no removal)
    const lenis = new Lenis({
      duration: isMobile ? 1 : 1.2, // slightly tighter for mobile
      easing: (t) => Math.min(1, 1.001 - Math.pow(2, -10 * t)),
      smoothWheel: true,
      touchMultiplier: isMobile ? 1.5 : 2, // smoother touch
    });
    ...
    const ctx = gsap.context(() => {
      ...
      containers.forEach((container, index) => {
        ...
        headings.forEach((heading) => {
          if (!heading.dataset.split) {
            ...
            gsap.fromTo(
              ...
              {
                fontWeight: 900,
                stagger: isMobile ? 0.015 : 0.05, // ✅ smoother mobile
                ease: "power2.out",
                scrollTrigger: {
                  trigger: container,
                  start: isMobile ? "top 92%" : "top 95%",
                  end: isMobile ? "top 50%" : "top 30%",
                  scrub: isMobile ? 0.6 : true, // ✅ smoother scrub
                },
              }
            );
          }
        ...
        gsap.fromTo(
          ...
          { xPercent: isEven ? 0 : -50 },
          {
            xPercent: isEven ? -40 : -10, // ✅ reduced movement mobile
            ease: "none",
            scrollTrigger: {
              trigger: container,
              start: "top bottom",
              end: "bottom top",
              scrub: isMobile ? 0.6 : 1, // ✅ smoother
            },
          }
        );
      });
