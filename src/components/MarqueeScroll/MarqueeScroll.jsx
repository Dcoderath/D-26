...
const MarqueeScroll = () => {
  ...
  useEffect(() => {
    ...
    const checkReady = () => {
      if (loadedCount === imgs.length) setIsReady(true);
    };
    ...
    imgs.forEach((img) => {
      if (img.complete) {
        ...
        img.onload = () => { loadedCount++; checkReady(); };
        img.onerror = () => { 
          loadedCount++; 
    ...
    const ctx = gsap.context(() => {
      const containers = containerRef.current.querySelectorAll(".marqueeScroll-container");
